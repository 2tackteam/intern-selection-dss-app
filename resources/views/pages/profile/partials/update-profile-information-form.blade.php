<section>
    <header>
        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
            {{ __('profile.profile_info.title') }}
        </h2>

        <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
            {{ __('profile.profile_info.subtitle') }}
        </p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6">
        ...
        <div>
            ...
            @if ($user instanceof \App\Models\User && ! $user->hasVerifiedEmail())
                <div>
                    <p class="text-sm mt-2 text-red-600 dark:text-red-300">
                        {{  __('profile.messages.unverified') }}
                    ...
                    @if (session('status') === 'verification-link-sent')
                        <p class="mt-2 font-medium text-sm text-green-600 dark:text-green-400">
                            {{ __('profile.messages.verification_link_sent') }}
                        </p>
                    @endif
                </div>
            @endif
