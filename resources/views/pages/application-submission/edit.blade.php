...
@php use App\Enums\YearEnum; @endphp
@php use App\Enums\EducationLevelEnum;use App\Models\Application; @endphp
@if(!isset($data))
    @dd('Something wrong!')
@endif
...
<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            {{ __('application-submission.edit.page_title') }}
        </h2>
    </x-slot>
            ...
            <div class="p-4 sm:p-8 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                <div class="sm:max-w-full md:max-w-xl">
                    ...
                    @if($data['application'] instanceof Application)
                        ...
                        <x-link-button class="mb-4" href="{{ route('application-submissions.show', hashIdsEncode($application->id)) }}">
                        ...
                        <section class="mt-3">
                            <header>
                                <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">
                                    {{ __('application-submission.edit.title') }}
                                </h2>

                                <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
                                    {{ __('application-submission.edit.subtitle') }}
                                </p>
                            </header>


                            <form method="post"
                                  action="{{ route('application-submissions.update', hashIdsEncode($application->id)) }}"
                                ...
                                <div>
                                    ...
                                    <x-text-input id="name" name="full_name" type="text" class="mt-1 block w-full"
                                                  :value="old('full_name', $application->full_name)" autofocus/>
                                ...
                                <div class="w-full">
                                    ...
                                    <x-select-option name="gender">
                                        @foreach(GenderEnum::toArray() as $value)
                                            <option
                                                value="{{ $value }}" @selected(old('gender', $application->gender) == $value)>{{ __('application-submission.gender.'.$value) }}</option>
                                        @endforeach
                                    </x-select-option>
                                ...
                                <div>
                                    ...
                                    <x-select-option name="education_level">
                                        @foreach(EducationLevelEnum::toArray() as $value)
                                            <option
                                                value="{{ $value }}" @selected(old('education_level', $application->education->education_level) == $value)>{{ $value }}</option>
                                        @endforeach
                                    </x-select-option>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="institution_name" name="institution_name" type="text"
                                                  class="mt-1 block w-full"
                                                  :value="old('institution_name', $application->education->institution_name)"
                                                  autofocus/>
                                ...
                                <div>
                                    ...
                                    <x-text-input id="major" name="major" type="text" class="mt-1 block w-full"
                                                  :value="old('major', $application->education->major)" autofocus/>
                                    ...
                                    <div class="flex space-x-2.5">
                                        <div class="w-full">
                                            ...
                                            <x-select-option name="start_year">
                                                @foreach(YearEnum::toArray() as $value)
                                                    <option
                                                        value="{{ $value }}" @selected(old('start_year', $application->education->start_year) == $value)>{{ $value }}</option>
                                                @endforeach
                                            </x-select-option>
                                        ...
                                        <div class="w-full">
                                            ...
                                            <x-select-option name="end_year">
                                                @foreach(YearEnum::toArray() as $value)
                                                    <option
                                                        value="{{ $value }}" @selected(old('end_year', $application->education->end_year) == $value)>{{ $value }}</option>
                                                @endforeach
                                            </x-select-option>
                                    ...
                                    <div class="flex space-x-2.5">
                                        <div class="w-full">
                                            ...
                                            <x-text-input id="gpa" name="gpa" type="number" step="0.01"
                                                          class="mt-1 block w-full"
                                                          :value="old('gpa', $application->education->gpa)" autofocus/>
                                            <x-input-error class="mt-2" :messages="$errors->get('gpa')"/>

                                            <small
                                                class="text-neutral-700">*{{ EducationLevelEnum::SHS_VHS->value }}
                                                = 0 s/d 100</small>
                                            <br>
                                            @php($educations = EducationLevelEnum::toArray())
                                            @php(array_shift($educations))
                                            <small class="text-neutral-700">*{{ implode(', ', $educations) }} = 0 s/d
                                                4.0</small>
                                        </div>
                                        ...
                                        <div class="w-full hidden" id="gpa_equal_section">
                                            ...
                                            <x-text-input id="gpa_equal" name="gpa_equal" type="number" step="0.01"
                                                          class="mt-1 block w-full disabled:bg-gray-300 disabled:border-gray-100 disabled"
                                                          disabled autofocus/>
                                        </div>
                                    </div>
                                </div>
                    ...
                    @pushonce('child-scripts')
                        ...
                        <script>
                            const SHS_VHS = @js(EducationLevelEnum::SHS_VHS->value);
